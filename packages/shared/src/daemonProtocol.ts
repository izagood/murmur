...
export type DaemonErrorCode =
  ...
  | 'no-such-runner'
  /**
   * 그 에이전트의 **앞 세대 러너가 아직 물러나는 중**이다(2026-09-07 후속).
   *
   * 실패가 아니라 **순서**다. 낡은 세대의 러너는 `SIGTERM` 을 드레인으로 받아 진행 중인
   * 턴을 끝내고 나간다(`agent/src/main.ts`). 그 전에 교체 러너를 띄우면 둘이 같은 inbox 를
   * 폴하고 아직 `markRead` 안 된 멘션을 둘 다 답한다 — `#430`·`#174` 가 싸운 중복이다.
   *
   * 그래서 daemon 이 "아직이다"라고 말하고, 앱은 **기다렸다 다시 부른다.** `internal` 로
   * 뭉치면 앱은 기다릴 줄 모르고 그 에이전트를 실패로 표시한다.
   */
  | 'retiring'
...
export interface RunnerInfo {
   ...
   *
   * ## `null` 은 "아무도 종료를 요청하지 않았다"가 아니다
   *
   * 종료 요청 경로가 **둘**이고 이 필드는 그중 하나만 안다:
   *
   * | 출처 | 어디에 기록되나 | daemon 이 아는가 |
   * |---|---|---|
   * | 사람이 UI 에서 | 서버 DB 의 `stopRequestedAt`(`#428`) | **모른다** |
   * | daemon 이 시그널로 | 이 필드 | 안다 |
   *
   * 사람이 UI 로 요청해 러너가 물러나는 중이어도 daemon 입장에서는
   * `termSentAtMs: null, alive: true` 다 — **두 필드 다 daemon 자신의 행위만 기록한다.**
   * 그래서 화면에서 이런 조합이 나올 수 있고, **둘 다 사실이다**:
   *
   * ```
   * 서버 쪽 : 종료 요청함 (10:23) — 러너가 읽어 갔다
   * daemon  : alive, termSentAtMs 없음
   * ```
   *
   * 나란히 놓으면 모순처럼 읽히므로, 화면은 **어느 쪽이 요청했는지 함께 보여야 한다**(`#443`).
   *
   * ## 무엇에 쓰나
   *
