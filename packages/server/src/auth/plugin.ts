...
declare module 'fastify' {
  interface FastifyRequest {
    ...
    /** 이 요청을 인증한 토큰의 해시. WS 티켓을 "어떤 자격증명으로 발급됐는지"에 묶는 데 쓴다. */
...
export async function registerAuth(app: FastifyInstance, pool: Pool): Promise<void> {
  ...
  app.addHook('onRequest', async (req) => {
    ...
    if (viaSession.rowCount) { req.account = viaSession.rows[0]; req.tokenHash = hash; return; }
    ...
    if (viaPat.rowCount) { req.account = viaPat.rows[0]; req.tokenHash = hash; }
  });
