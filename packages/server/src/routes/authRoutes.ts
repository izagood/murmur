...
export async function registerAuthRoutes(app: FastifyInstance, pool: Pool): Promise<void> {
  ...
  // 로그아웃은 **이 토큰만** 끊는다. 다른 기기의 세션은 유지된다 — 한 기기에서 나가는 것이
  // 모든 기기에서 쫓겨나는 것과 같으면 놀라움이 크다. 전체 폐기가 필요하면 별도 표면이어야 한다.
  // 지금까지는 클라이언트가 로컬 토큰만 지웠고 서버 세션은 14일을 더 살았다.
  app.post('/auth/logout', { preHandler: app.requireAccount }, async (req, reply) => {
    await pool.query(`delete from session where token_hash = $1`, [req.tokenHash]);
