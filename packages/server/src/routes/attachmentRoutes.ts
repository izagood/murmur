...
export async function registerAttachmentRoutes(
  ...
  app.get('/attachments/:id', { preHandler: app.requireAccount }, async (req, reply) => {
    ...
    // 가시성 생각은 `resolveAttachmentFor` 한 곳에만 있다 — MCP `attachment.fetch` 도 그것을
    // 부른다(#585). 여기에 인라인으로 다시 쓰면 한족만 고친 날에 한쪩이 새는다.
    const resolved = await resolveAttachmentFor(pool, id, req.account!.id);
    // 문장까지 옛것과 같게 답한다 — 이 변경은 판정을 옮기는 것이고, 응답은 그대로여야 한다.
    if (!resolved.ok) {
