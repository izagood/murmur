...
export interface AttachTicketClaim extends TicketClaim {
  ...
  sessionId: string;
  // 쓰기 자격은 여기 없다 — 티켓은 attach **인가**만 운반하고, 쓰기 **차례**(지금 누가
  // writer 인가)는 허브가 산다(스펙 §5-2 결정 2, `ws/relay.ts::setWriter`). 한때 있던
  // `canInput` 은 attach 시점의 판정을 소켓 수명 내내 얼려 버려, "마지막 attach 가
  // writer" 라는 시간에 따라 움직이는 사실을 담을 수 없었다.
}
