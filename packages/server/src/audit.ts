...
export type AuditAction =
  ...
  // #315: 사람이 그 터미널에 **타이핑해 개입했다.** 남기는 것은 사실뿐이다 —
  // 누가(actorId), 언제(at), 어느 턴에(detail.sessionId). **친 내용은 절대 넣지 않는다**:
  // 사람이 치는 것에도 비밀이 섞인다(붙여 넣은 토큰, 비밀번호 프롬프트의 답), 그리고
  // PTY 출력을 DB 에 안 남기기로 한 이유(스펙 §5)가 입력에도 그대로 적용된다.
  //
  // **연타마다 한 행을 만들지 않는다.** 사람이 한 턴에 백 번 키를 누르면 감사가 백 행이
  // 되고, 그러면 감사 조회에서 다른 사건이 전부 밀려난다 — 감사가 스스로를 못 쓰게 만든다.
  // 한 attach 세션의 첫 입력에 한 번 남기고, 그 뒤로는 일정 간격(라우트의
  // `INPUT_AUDIT_WINDOW_MS`)마다 한 번만 남긴다. 답해야 하는 질문이 "몇 글자를 쳤나"가
  // 아니라 "이 턴에 사람이 개입했나" 이므로, 묶어도 그 질문의 답은 잃지 않는다.
  | 'agent.input';
