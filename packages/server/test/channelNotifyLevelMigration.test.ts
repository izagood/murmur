...
/**
 * #224 회귀선 — 024 마이그레이션이 **이미 음소거한 채널을 `none` 으로 옮기는가**, 그리고
 * **나머지를 시끄럽게 만들지 않는가.**
 *
 ...
 * 유일한 방법이다 — 이미 마이그레이션이 끝난 DB 에 행을 넣으면 컬럼 default 가 붙어서
...
describe('024_channel_notify_level', () => {
  it('moves an existing mute to none and leaves the rest at mentions', async () => {
    // 024 이전 상태로 되돌린다.
    ...
    // 음소거하지 않은 채널은 024 이전 동작 그대로 — "나를 부른 것만" 이다. 여기가 'all' 이면
    // 마이그레이션이 아무도 고르지 않은 "모든 메시지 알림"을 밀어 넣는다.
    expect(byChannel[plainChannelId]).toBe('mentions');
  });

  it('defaults a brand new pref row to mentions', async () => {
    ...
    expect(res.rows[0].notify_level).toBe('mentions');
  });
