// murmur daemon 의 엔트리포인트 — **러너를 소유하는 상주 프로세스**다(`#431` 2단계-b).
//
// 2단계-a 까지 이 파일은 인자를 적고 바로 끝났다. 이제는 엔드포인트를 획득하고, 소켓을
// 열고, 러너를 띄우고, 죽을 때 **러너를 데려가지 않고** 물러난다.
//
// ## 여기서 하지 않는 것 (범위)
//
// - **고아 재발견**(`adoptRunner`) — 2-c. daemon 이 재시작하면 이전 러너를 다시 알아보지
//   못한다. 그때까지 그 정리는 사람이 한다(`#431` 코멘트)
