...
**옮겨 간 것**: *"`external` 판정을 앱에서 daemon 으로"* 는 여기 "아직 없는 것"에 있었는데
`#482` 가 했다. `external`(서버 presence 로 지은 추측)은 없어졌고 지금 값은 `adopted` —
**이 daemon 의 장부에 있고 `kill(pid, 0)` 으로 살아 있음을 확인한** 러너다. 이 패키지가
그 판정의 근거를 쥔다. 화면 문구는 여전히 `#443` 이다.
