...
describe('#337 MentionQueue', () => {
  it('처음 유예된 entry 는 통지 대상이고, 같은 entry 의 재폴링은 아니다', () => {
    ...
    const first = q.defer('c1/m1', 10, 42, 1_000);
    expect(first).toEqual({ shouldNotify: true, pending: 1, heldMs: 0, warned: false });

    // inbox 는 at-least-once 라 같은 entry 가 다음 배치에 또 온다 — 통지가 또 붙으면
    // 조종이 길어질수록 스레드가 "대기 중" 도배가 된다.
    const again = q.defer('c1/m1', 10, 42, 4_000);
    expect(again).toEqual({ shouldNotify: false, pending: 1, heldMs: 3_000, warned: false });
  });

  it('N 은 entry 집합의 크기다 — 두 번째 멘션은 대기 2건째다', () => {
    const q = new MentionQueue();
    q.defer('c1/m1', 10, 42, 1_000);
    expect(q.defer('c1/m1', 11, 45, 1_000))
      .toEqual({ shouldNotify: true, pending: 2, heldMs: 0, warned: false });
  });

  it('minSeq 는 대기 멘션의 최소 seq 다 — lastFedSeq 클램프의 재료(§5-2 결정 7)', () => {
    const q = new MentionQueue();
    expect(q.minSeq('c1/m1')).toBeNull();
    q.defer('c1/m1', 11, 45, 1_000);
    q.defer('c1/m1', 10, 42, 1_000);
  ...
  it('인터랙티브가 끝나면 key 를 지운다 — 다음 조종 때 통지가 다시 1회부터 시작한다', () => {
    ...
    expect(q.defer('c1/m1', 10, 42, 9_000))
      .toEqual({ shouldNotify: true, pending: 1, heldMs: 0, warned: false });
  });
});
