...
**예외는 깨움이다.** 그 턴에 `turn.wake`로 기다림을 예약했으면 통지를 남기지 않는다 —
스레드에 대기 줄이 이미 보이므로 사람은 무슨 일인지 안다(`prompt.ts::hasOwnWakeSince`).

## 턴은 한 번이고, 기다림은 예약이다

`claude -p` 는 모델이 말을 멈추는 순간이 프로세스 종료다. 백그라운드로 띄운 명령도 그때
함께 죽는다 — 2026-09-07 15:08 에 PR #533 을 올린 턴이 CI 대기 루프를 백그라운드로 띄우고
"결과 나오면 머지하겠다"며 끝나, 4분 뒤 초록이 된 CI 를 아무도 보지 않았다. 시스템 프롬프트는
"어디에 쓸지"(`message.post`)만 말하고 **언제까지 살아있는지**를 말하지 않았고, 그 공백이
실행 불가능한 계획을 낳았다.

그래서 프롬프트는 이제 세 가지를 함께 말한다: 멈추면 죽는다 · 이 턴의 예산은 N분이다
(`config.turnTimeoutMs`) · 더 기다려야 하면 `turn.wake` 로 예약하고 끝내라.

깨움의 생김새(서버 쪽은 마이그레이션 040 의 주석이 자세히 적는다):

1. 에이전트가 `turn.wake(channelId, threadRootId, notBeforeSec, reason)` 를 부른다.
2. 서버가 그 자리에서 `kind='wake'` 메시지를 게시한다 — **지금 보인다**. 사람이
   "죽었나 기다리나"를 아는 근거가 이 줄이다.
3. 시각이 되면 sweeper 가 그 메시지로 **작성자 자신의** inbox 항목(`reason='wake'`)을 만든다.
4. 러너의 `inbox.poll` 이 그것을 받고, 앵커(그 메시지의 `threadRootId`)로 **같은 세션을
   `-r` 로 재개**한다. 조사한 것을 다시 조사하지 않는다.

한계: 깨움은 스레드 안에서만 걸 수 있다(앵커가 세션 키다). 하한 60초·연속 20회 상한이
있고, 소진되면 `message.fail(retryable: true)` 로 사람에게 넘겨야 한다.
