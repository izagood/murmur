...
/**
 * **오늘까지의 화면이 서 있던 칸.** 이 칸에서 웹뷰 배율이 1.0 이다.
 *
 * 100 이 아니라 90 인 이유: murmur 의 본문단은 13px 인데 같은 종류의 앱(Slack)이 15px 이라
 * 지금 화면은 이미 통상의 ~87% 다. 그 상태를 "100%" 라고 부르면, 배율을 붙여도 기본값으로
 * 켠 사람은 **여전히 작은 화면**을 보게 되고 이 작업이 고치려던 것이 안 고쳐진다. 그래서
 * 눈금의 기준을 통상 크기에 맞추고, 오늘의 화면은 그 90% 칸에 그대로 남겨 둔다 — 지금
 * 크기가 좋았던 사람은 90% 를 고르면 한 픽셀도 달라지지 않는다.
 *
 * 즉 이 상수가 하는 일은 **눈금의 원점을 옮기는 것**이지 화면을 늘리는 것이 아니다.
 * 늘리는 것은 아래 `DEFAULT_ZOOM` 이 100 이라는 사실이다.
 */
export const BASELINE_ZOOM = 90;

/** 기본 배율. `BASELINE_ZOOM` 과 한 칸 차이라, 새로 켜는 사람은 오늘보다 약 11% 큰 화면을 본다. */
