import { CHANNEL_MENTION_HANDLE, denormalizeMentions, fillSystemAccount, MENTION_PATTERN, MENTION_TOKEN_PATTERN, mentionedHandles, mentionScope, type MessageRow, renderMentions } from '@murmur/shared';
 ...
 *
 * 코드 블록(#298)과 인용 줄(#597) 안의 `@handle` 은 여기서 **잡히지 않는다**. 이 줄은 알림이 실제로 가는
 * 쪽을 따라야 하고, 서버도 `mentionScope` 로 코드·인용(#597) 안을 제외한다 — 그러므로 이
...
export function bodyRecipients(
  ...
  for (const part of splitMentions(mentionScope(body), knownHandles, groupHandles)) {
