import { CHANNEL_MENTION_HANDLE, denormalizeMentions, fillSystemAccount, MENTION_PATTERN, MENTION_TOKEN_PATTERN, mentionedHandles, mentionSearchText, type MessageRow, renderMentions } from '@murmur/shared';
 ...
 *
 * 코드 블록(#298)·인용(#593) 안의 `@handle` 은 여기서 **잡히지 않는다**. 이 줄은 알림이 실제로
 * 가는 쪽을 따라야 하고, 서버도 `mentionSearchText` 로 같은 구간을 제외한다 — 그러므로 이
...
export function bodyRecipients(
  ...
  for (const part of splitMentions(mentionSearchText(body), knownHandles, groupHandles)) {
