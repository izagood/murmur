...
interface Props {
  ...
  scopeKey?: string;
  /**
   * 이 작성창이 속한 채널(#173). 채널이 자동으로 멘션하는 에이전트를 스토어에서 찾는 열쇠다.
...
export function Composer({ onSend, placeholder, rows = 2, autoFocus, scopeKey = '', channelId }: Props) {
  ...
  const pickable = useMemo(
    () => Object.values(accounts)
      .filter((a) => a.id !== myId && !sticky.includes(a.handle.toLowerCase()) && !autoHandles.includes(a.handle.toLowerCase()))
      .sort(rank)
      .slice(0, MAX_SUGGESTIONS),
    [accounts, myId, sticky, autoHandles],
  );
