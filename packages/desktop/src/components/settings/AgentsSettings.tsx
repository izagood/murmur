...
export function AgentsSettings({ targetId }: { targetId?: string }) {
  ...
  /**
   * 화면의 **중지** 버튼(#129, 어휘는 #491). 러너를 지금 끊는 것이 아니라 "진행 중인 턴을
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="w-full max-w-2xl flex-1 space-y-4 overflow-y-auto p-5">
            ...
            */}
            ...
            {selected && isAdmin && (
              <div className="rounded border border-border p-3">
                {/* #129 → #427 → #491: "재시작"을 금지한 원칙은 그대로 살아 있고, **사실관계만
                ...
                {/* #491: 버튼이 "종료 요청"/"요청 되돌리기" 둘에서 **한 자리 토글**로 접혔다.
                ...
                {/* #491: **세 상태를 버튼이 아니라 이 상태 표시로 옮겼다.**
                ...
                <div className="mt-2 text-[11px]" role="status">
                  ...
                  {selected.stopRequestedAt && selected.stopAckedAt && (
                    <span className="text-fg-muted">
                      ...
                      {/* #427 → #491: 러너가 이미 읽어 간 뒤가 오히려 다시 켤 필요가 생기는
                ...
                {/* #491: 켜는 길과 끄는 길을 **한 자리**에 겹쳐 둔다. `#427` 이 "되돌리는 길을
