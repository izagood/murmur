 ...
 *
 * **여기 서는 것은 실패뿐이다.** 성공을 태우지 않는다 — 경고색이고 저절로 사라지지 않아
 * 사람이 ×를 눌러야 하므로, 잘 된 일을 실으면 창 제일 위가 덮이고 그것을 손으로 치워야
 * 한다. 'Link copied.' 가 그래서 빠졌다(복사는 붙여넣으면 됐는지 바로 안다).
 *
 * 별도 컴포넌트인 이유: 알림을 만드는 곳(컨트롤러의 `openMessage`, 메시지의 클립보드 실패)과
