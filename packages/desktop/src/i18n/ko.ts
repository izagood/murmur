...
export const ko = {
  ...
  'agentTurns.endControlDetail': '터미널이 닫히고 하네스가 멈춘다. 거기서 친 것은 이미 세션에 남아 있어 터미널을 다시 열면 이어진다. 러너는 살아 있고, 이 스레드가 붙잡고 있던 멘션이 곧 처리된다.',
  'agentTurns.endControlConfirm': '조종 끝내기',
  ...
  'agentTurns.startUnknown': '시작 시각 모름',
  'agentTurns.subline': '{n}개 도는 중 \u00b7 스레드 {threads}곳 \u00b7 가장 오래된 것 {oldest}',
  'agentTurns.terminal': '터미널',
  'agentTurns.terminalTitle': '이 턴의 터미널을 연다',
  ...
  'collab.title': '협업',
  'collab.checking': '제안을 불러오는 중…',
  'collab.unknown': '제안을 읽지 못했다',
  'collab.noServer': 'avcs 서버가 설정되지 않았다 — 설정 › 투영에서 정한다.',
  'collab.empty': '여기에는 아무것도 없다',
  'collab.filter.label': '제안 거르기',
  'collab.filter.open': '열림 {count}',
  'collab.filter.accepted': '승인됨 {count}',
  'collab.filter.all': '전부',
  'collab.openChannel': '채널 열기',
  'collab.repoUnreachable': '이 저장소를 읽지 못했다',
  'collab.reducedAt': '#{cursor} 시점 · {materializer}',
  'collab.ownerUnknown': '모름',
  'collab.ops': 'op {count}',
  'collab.effects.behavior': '동작 바뀜',
  'collab.effects.api': '공개 API 깨짐',
  'collab.conflict': '충돌 — {key}',
  'collab.detail.ops': '작업',
  'collab.detail.noOps': '아직 op 이 없다 — 열어만 둔 제안이다',
  'collab.detail.conflicts': '충돌',
  'collab.detail.decisions': '결정',
  'collab.opStatus.proposed': '올림',
  'collab.opStatus.validating': '검증 중',
  'collab.opStatus.accepted': '받아들임',
  'collab.opStatus.rejected': '거부됨',
  'collab.opStatus.superseded': '대체됨',
  'collab.opStatus.needs_decision': '결정 필요',
  'collab.opStatus.quarantined': '격리됨',
  'collab.state.needs_decision': '결정 필요',
  'collab.state.check_failed': '검증 실패',
  'collab.state.open': '열림',
  'collab.state.accepted': '승인됨',
  'collab.state.rejected': '거부됨',
  'collab.state.unknown': '상태 모름',
  'community.version.value': '서버 v{version}',
  'community.version.unknown': '서버 버전 확인 중',
  'community.version.legacy': '이 서버는 버전을 말하지 않는다',
  'community.version.legacyDetail': '버전을 싣기 전 판이다 — 재배포하면 무슨 빌드가 도는지 보인다.',
  'community.version.incompatible': '서버 v{version} — 이 앱이 요구하는 버전보다 낮다',
  'community.version.incompatibleDetail': '이 앱은 서버 v{minVersion} 이상이 필요하다. 재배포하기 전까지 일부 기능이 동작하지 않는다.',
  'community.version.behindDetail': '이 앱(v{appVersion})보다 뒤처졌지만, 요구 버전(v{minVersion})은 넘는다.',
  'community.version.aheadDetail': '이 앱(v{appVersion})보다 앞선다.',
  'community.version.started': '{ago} 기동',
  'community.version.commit': '빌드 {commit}',
  'community.compat.bannerText': '이 서버(v{version})는 이 앱에 너무 낮다',
  'community.compat.bannerDetail': 'v{minVersion} 이상으로 재배포해라 — 그전까지 일부 기능이 동작하지 않는다.',
  'community.compat.bannerOpen': '설정 열기',
  'community.compat.bannerDismiss': '닫기',
  ...
  // ---------------------------------------------------------------------------
  // reactions — 칩 호버의 이름 목록. 근거는 `en.ts` 의 같은 구획에.
  // ---------------------------------------------------------------------------

  'reactions.you': '나',
  'reactions.unknown': '누군가',
  /** 한국어는 수에 따라 명사가 안 바뀌므로 `other` 하나로 충분하다(`ko.ts` 머리말). */
