...
describe('SettingsScreen', () => {
  ...
  it('can open straight into a requested section', () => {
    render(<SettingsScreen initialSection="updates" onBack={vi.fn()} onSignOut={vi.fn()} />);
    expect(screen.getByRole('heading', { name: 'Updates' })).toBeTruthy();
  });

  it('returns to the app', () => {
    const onBack = vi.fn();
    render(<SettingsScreen onBack={onBack} onSignOut={vi.fn()} />);
  ...
  it('passes sign-out through from a section', () => {
    ...
    render(<SettingsScreen onBack={vi.fn()} onSignOut={onSignOut} />);
