...
describe('사이드바 접기', () => {
  it('접으면 사이드바가 사라진다', () => {
    fakeController();
    const { container } = render(
      <Sidebar onLogout={vi.fn()} onOpenSettings={vi.fn()} collapsed={true} onToggleCollapse={vi.fn()} />
    );
  ...
  it('접을 때 토글 콜백이 불린다', () => {
    ...
    const { container } = render(
      <Sidebar onLogout={vi.fn()} onOpenSettings={vi.fn()} collapsed={false} onToggleCollapse={onToggleCollapse} />
    );
