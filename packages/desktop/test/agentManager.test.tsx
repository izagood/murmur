...
/**
 * 러너 실행·중지(#129, 어휘는 #491). 화면이 말할 수 있는 것은 **세 가지**뿐이다:
 ...
 *
 * `#491` 이 버튼 자리를 하나로 접었어도 **이 세 상태는 그대로 그린다** — 접힌 것은 버튼이지
 * 상태가 아니다. 그래서 아래 단언들도 이름만 갈고 그대로 둔다.
 */
describe('러너 실행·중지 (#129, #491)', () => {
  ...
  it('어느 상태에서도 멈췄다고 단정하지 않는다', async () => {
    ...
    for (const state of states) {
      ...
      /**
       * 프로세스의 생사를 단정하는 문구를 막는다. **이 가드는 `#491` 뒤에도 그대로다** —
       * 버튼이 "중지"가 되어도 murmur 는 여전히 프로세스의 생사를 모른다(러너가 종료하면
       * 다음 GET /agent/config 자체가 오지 않는다).
       *
       * `재시작` 은 계속 막는다. `#491` 이 허용한 것은 **"실행"** 이지 "재시작"이 아니다 —
