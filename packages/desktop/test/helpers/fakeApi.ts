...
export const msg = (id: string, channelId: string, seq: number, body: string, authorId = 'u1',
  ...
  ({ id, seq, channelId, threadRootId: null, authorId, body, kind: 'user', meta: {}, createdAt: new Date().toISOString(), editedAt: null, reactions: [], attachments: [], replyCount: null, activityCount: null, lastReplyAt: null, participantIds: null, openAskHumanCount: null, openAskAccountIds: null, openAskLinks: null, failureCount: null, unresolvedFailureCount: null, lastKind: null, lastAuthorId: null, alsoInChannel: false, deletedAt: null, ...extra });
...
export function fakeApi(overrides: Partial<ApiClient> = {}): ApiClient {
  const base = {
    ...
    // 반대 방향도 **같은 메시지**를 돌려준다 — 새 메시지가 아니므로 seq 도 그대로다.
    postToChannel: vi.fn(async (_c: string, messageId: string) =>
      msg(messageId, 'c1', 7, '흘린 말', 'u1', { threadRootId: 'm1', alsoInChannel: true })),
