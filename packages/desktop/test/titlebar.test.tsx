 ...
 *
 * 레일은 가로로 비우지 않는다 — 신호등 3개(78px)보다 좁아서 `pl-[78px]` 로는 피할 수 없다.
 * 그래서 이 파일의 단언은 `MAC_TRAFFIC_LIGHT_PL` 이 **두 바에 없다**는 쪽이다.
 *
 * **레일이 비우는 방식이 `pt-8` 에서 타이틀바 띠로 바뀄다**(2026-09-08, #4). 그 여백은
 * 색이 없어서 신호등이 레일 면 위에 그냥 떠 있었고, 창 맨 위 한 줄이 사이드바에서
 * 시작하는 것처럼 보였다. 이제 레일 맨 위에 `TOP_BAR_H` 높이의 띠(`rail-titlebar`)가
 * 서서 신호등 자리와 타이틀바를 함께 진다 — 그래서 이 묶음은 `pt-8` 대신 **띠가 있는지**를
 * 재고, `macOS 가 아니어도` 띠는 그대로다(띠는 신호등을 피하는 여백이 아니라 앱이 스스로
 * 그리는 타이틀바이고, 다른 두 조각은 이미 모든 플랫폼에서 선다).
 */
describe('#270 신호등 여백', () => {
  it('macOS·사이드바 펼침 — 좌상단은 레일이라 브랜드 바도 헤더도 여백을 지지 않는다', () => {
    ...
    // 여백을 잃은 것이 아니라 옮긴 것이다 — 레일 맨 위의 띠가 그 자리를 진다.
  ...
  it('macOS 가 아니어도 띠는 그대로다 — 띠는 여백이 아니라 타이틀바다', () => {
    pretendWindows();
    renderWorkspace({ sidebarCollapsed: false });
    expect(screen.getByTestId('sidebar-brand').className).not.toContain(MAC_TRAFFIC_LIGHT_PL);
    expect(screen.getByTestId('app-header').className).not.toContain(MAC_TRAFFIC_LIGHT_PL);
    expect(screen.getByTestId('rail').className).not.toContain('pt-8');
    // 브랜드 바·헤더가 모든 플랫폼에서 서므로 레일 쪽 조각도 그렇다 — 한 줄이 창 폭
    // 전체에서 하나여야 하고, 플랫폼마다 다르면 그 줄이 두 가지 모양을 갖는다.
    expect(screen.getByTestId('rail-titlebar').className).toContain(TOP_BAR_H);
