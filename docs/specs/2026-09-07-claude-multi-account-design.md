...
### 로그인만으로는 부족하다 — 첫 실행 승인을 함께 통과시킨다 (2026-09-08)

**그 창에서 나오기 전에, 뜨는 승인 화면을 전부 통과하고 입력창이 보이는 것까지 확인한다.**

멘션 턴이 TUI 로 돌기 시작하면서(`2026-09-08-tui-execution-model-design.md`) 이것이 필수가
됐다. `-p` 는 이 승인들을 **설계상 건너뛴다**(claude `--help`: *"The workspace trust dialog is
skipped when Claude is run in non-interactive mode"*). TUI 는 사람이 앉아 있다고 전제하므로
묻고 기다린다 — 그리고 러너는 답할 수 없다.

실물에서 관측한 것 셋(2026-09-08):

| 승인 화면 | 언제 뜨나 |
|---|---|
| 온보딩 — `Select login method:` | 그 config 디렉터리에 `hasCompletedOnboarding` 이 없을 때 |
| 팀 텔레메트리 — `Yes, I trust these settings` | 팀 admin 이 모니터링을 켜서 조직 설정이 그 계정에 동기화됐을 때 |
| 폴더 신뢰 — `Yes, I trust this folder` | 처음 보는 작업 디렉터리에서 (스레드 워크스페이스는 **러너가 적는다**, 아래) |

**왜 러너가 대신 하지 않는가.** 위 둘은 계정 수준 상태이고, 그것을 러너가 심으면 하네스의
내부 상태 기계를 흉내내는 일이 된다 — 판본·조직 설정마다 새 승인이 생기고 그때마다 러너가
조용히 멈춘다. 계정 등록은 이미 사람의 일이므로(이 절이 그 근거다) 승인도 여기서 함께 지난다.

**예외 하나**: 스레드 워크스페이스의 폴더 신뢰는 사람이 미리 답할 수 없다 — 스레드마다 새로
만들어지기 때문이다. 그것만 러너가 적는다(`workspaceTrust.ts`).

**승인을 안 지나면 어떻게 보이나**: 그 계정으로 도는 턴이 준비 신호를 못 봐 상한(60초)에서
`PromptNotDeliveredError` 로 실패한다. 러너 로그의 `준비 신호를 …ms 안에 못 봤다` 가 그 줄이다.
