...
## 7. 단계 — 실제로 어떻게 갔나 (2026-09-11 갱신)

계획한 P0~P5 와 **순서가 달라졌다.** 달라진 근거를 남긴다 — 다음 하네스를 붙이는 사람이
이 표를 따라가게 되므로, 계획이 아니라 **한 일**이 적혀 있어야 한다.

| | 한 일 | PR |
|---|---|---|
| ✅ | 계약 + 두 어댑터 + 패리티 (호출부 무변화) | #763 |
| ✅ | **세 번째 하네스(opencode) 실측** → 계약 2곳 수정 | #765 |
| ✅ | opencode 어댑터 · `certify` · 이름 비교 **예산 테스트** → 계약 2곳 더 수정 | #767 |
| ✅ | 세션 기록 판정 이설(네 자리가 한 질문이었다) | #769 |
| ✅ | 턴의 세 사실 이설 + **CI 가 양쪽 경로를 돈다** | #770 |
| ✅ | 화면: codex 에이전트에 claude 계정 풀을 그리지 않는다 | #772 |
| ✅ | **① codex 멘션 턴 TUI** (스위치 **밖**) | #774 |
| ✅ | **② 스위치 기본값 켜짐** (끄는 길 `=0` 남김) | #780 |
| ⬜ | **③ 옛 분기 · 스위치 · CI 가드 삭제** ← 이설의 끝 | |
| ⬜ | `turn.ts` 의 `homeEnv` 통합 (네 번째 하네스의 홈) | |
| ⬜ | opencode 를 RUNNABLE 로 (`certify --turn` 부터) | |

### 순서가 달라진 세 가지

**1. 세 번째 하네스를 앞으로 당겼다(원래 P4).** 계약이 옳은지는 셋째를 붙여 봐야 알기
때문이다 — 둘로는 우연히 맞는다. 값을 했다: opencode 하나가 계약을 **네 자리** 고쳤다.
`account.configDirEnv` 가 배열이 된 것(XDG 셋), `transcript` 가 파일/CLI 갈래로 나뉜 것
(SQLite), `trust` 가 `| null` 이 된 것(신뢰 관문이 없다), `mcpRegistration` 에
`'account-config'` 가 붙은 것(턴별 수단이 없다). **계정 축을 먼저 일반화했다면 그 설계가
claude·codex 두 모양에만 맞춰졌을 것이다.**

**2. eslint 규칙 대신 예산 테스트를 먼저 뒀다(원래 P2).** `AGENT_HARNESSES` 에 이름을 더해도
**타입은 아무것도 안 잡는다**(실측): `harness === 'claude-code'` 는 유니온이 늘어도 그대로
컴파일되고 새 하네스는 조용히 모든 `else` 로 떨어진다. 그래서 컴파일러를 기대하지 않고
이름 비교 **개수를 파일별로 고정**했다(`adapterParity.test.ts`). 0 이 되는 날 규칙으로 바꾼다.

**3. codex TUI 를 스위치 **밖**에서 했다(①/② 분리).** 처음에는 codex TUI 를 새 경로에만
넣었는데, 그러면 스위치를 켜는 순간 *경로*와 *실행 방식*이 **동시에** 바뀌어 codex 턴이
깨졌을 때 어느 쪽 탓인지 가릴 수 없다. 갈라 놓으면 ①이 독립으로 검증되고 ②의 스위치는
**아무 동작도 바꾸지 않는 리팩터**가 되어 패리티로 증명된다.

그 분리가 곧바로 구멍 하나를 닫았다: 실물 CLI 대조(`acceptance-cli.test.ts`)가 **기본
경로**에서 돌게 됐다. ①이 새 경로에만 있던 동안에는 스위치를 켜야만 새 argv 가 검증됐다 —
프로덕션이 쓰는 경로가 아니라 켜야 보이는 경로를 재고 있었던 것이다.

### 전환의 규율 (러너 경로 일반)

이 경로가 깨지면 에이전트가 안 돌고, 그러면 murmur 자체를 못 쓴다. 그래서:
옛 경로를 **두고** 새 경로를 스위치 뒤에 만들고 → 행동으로 같음을 증명하고(+ 돌연변이로
테스트 이빨을 확인) → 하네스를 전부 돌려 보고 → 전환하고 → **그다음 릴리스에서** 옛 분기를
지운다. CI 가 매 PR 에서 **두 경로를 다** 돈다(전환 뒤에는 방향이 뒤집혀 롤백 경로를 지킨다).

## 8. 범위 밖

- ~~**desktop 의 계정 풀 UI 수정.**~~ **했다(#772).** 판단은 `@murmur/shared::harnessHasAccountPool`
  이 하고 — 이 사실을 러너와 데스크탑 **두 곳이 알아야** 해서 shared 에 뒀다 — 풀이 없는
  하네스에는 선택 대신 이유를 한 줄 그린다(감추면 사람이 원인을 지어낸다).
