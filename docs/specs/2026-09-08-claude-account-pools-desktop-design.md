...
## 12. §3 실측의 정정 (2026-09-08, 사용자 보고)

**§3 이 `claude auth status --json` 을 로그인 판정에 쓸 수 있다고 적은 것은 불완전했다.**
사용자가 앱에서 계정 넷을 등록한 뒤 설정 화면이 전부 `Not signed in` 으로 그렸다.
...
즉 **런타임은 Keychain 을 읽고 `auth status` 는 파일(`<configDir>/.credentials.json`)만
읽는다.** §3 의 실측이 이것을 놓친 이유는 분명하다: 그때의 심볼릭 링크 실험은 **파일이 있는**
경우였고, "Keychain 만 있는" 경우를 재지 않았다. 두 저장소가 갈릴 수 있다는 사실 자체가
§1 에 적혀 있었는데도(파일은 폴백이라고 적었다) 그 갈림을 판정에 반영하지 않았다.

**고친 방식**(`daemon/src/claudeAccounts.ts::accountStatusFromDisk`):

- **정체**는 `<configDir>/.claude.json` 의 `oauthAccount`(`emailAddress`·`organizationName`).
  비밀값이 아니다. **미로그인 디렉터리에는 이 키가 없다**(실측) — 존재 자체가 신호다.
- **자격증명**은 파일 또는 Keychain 항목의 **존재**. `security find-generic-password` 에
  `-w` 를 붙이지 않는다 — 존재 판정에 토큰을 읽을 이유가 없다.
- 부수 효과: 계정마다 `claude`(Node 앱) 프로세스를 하나씩 안 띄운다.

**남는 한계**: "자격증명이 있다"는 "토큰이 아직 유효하다"가 아니다. refresh 토큰이 만료된
계정도 화면에서는 로그인으로 보인다 — 그 사실은 턴을 돌려 봐야 알 수 있고, 그때 러너가
발견해 다음 계정으로 넘어간다(`switchesAccount` 가 `oauthsessionexpired` 를 잡는다).
...
**러너가 풀을 못 본 사건도 같은 날 관측했다.** 러너 넷이 01:35:19 에 떴고 계정은
01:37~01:39 에 만들어졌다. 러너는 풀을 기동 시 1회 읽으므로 전부
`claude 계정 풀이 비어 있다 (풀: 지정 없음)` 로 돌았다 — §5-2 가 적은 알려진 동작이고
UI 가 재시작을 안내하지만, **사용자에게는 "기본 풀이 할당되지 않았다"로 보였다.**
핫리로드를 후속으로 남긴 대가가 이것이다.
