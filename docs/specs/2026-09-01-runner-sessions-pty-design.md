...
스코프 = `(channelId, threadRootId)`. 채널 최상위-mention 은 그-mention 메시지 id 를
threadRootId 로 삼아 스레드를形成한다 — 이로써 두 가지를 달성한다:
1. **채널 가독성**: 답이 채널 본문에 쌓이지 않고 스레드에 가蓋된다.
2. **세션 격리**: 다른-mention 은 다른 session key 를 가져 서로의 맥락이 섞이지 않는다.
  (`_root` 로 뭉치던旧 설계는 새 사용자의 첫 진입점(채널에서 `@handle` 부르기)을
  오히려 오염시켰다). 스레드 안의-mention 은 기존 대 로 그 스레드의 루트 id 를 쓴다.
