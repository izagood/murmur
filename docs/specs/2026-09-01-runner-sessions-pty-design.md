...
1. `avcs workspace project murmur-<threadShort> --out <dir>` — 물리 격리.
...
3. 러너 로컬 상태 파일에 기록: `~/.murmur-agent/<agentHandle>/sessions.json` —
   `{threadKey: {workspaceDir, sessionId, harness}}`. **러너 머신의 사실은 러너 머신에
...
사람이 조종 중인 스레드에 멘션이 오면: 큐에 두고 스레드에 시스템 메시지 —
...
- **PAT 는 env 로 간다.** MCP 설정 파일에는 `${MURMUR_PAT}` 플레이스홀더만 두고
  ...
  않는다 — `ps` 에 PAT 가 뜬다.
- `--strict-mcp-config` 는 **쓰지 않는다** — 에이전트는 murmur MCP 외에 avcs MCP 를
  물고 일한다(도그푸딩 전제). strict 는 그것을 끊는다.
