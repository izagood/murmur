...
**2026-09-08 개정.** 멘션 턴은 **스레드마다 하나**이고, **스레드 간에는 무제한 병렬**로
돈다. 같은 스레드는 여전히 직렬이다(`TurnRegistry` + `mentionScheduler` 의 인플라이트 장부).
...
한도"* 는 지금도 참이다. 동시 상한을 두지 않기로 한 근거와 실측 메모리 비용(claude 프로세스
하나당 RSS 중앙값 약 190MB)은 위 설계 문서 §4-6 에 있다.
...
@scout 이 붙으면 러너가 서로 다른 프로세스이므로 둘은 병렬로 돈다(러너 안에서도 이제
스레드가 다르면 병렬이다 — 위 개정). 각자 자기 workspace·세션을 가지며(위 이름 규칙), 서로의 스레드 발화는
...
| 권한: auto | `--permission-mode bypassPermissions` | `-c sandbox_mode="workspace-write"` + `-c mcp_servers.murmur.default_tools_approval_mode="approve"` | — |
...
- **`codex exec` 에는 `-a`/`--ask-for-approval` 이 없다.** 권한은 sandbox 단독이고,
  ...
  거절돼 답이 채널에 도달하지 않았다(#404). auto 권한은 murmur MCP 서버의 공식 설정인
  `default_tools_approval_mode="approve"`만 턴별로 덧붙인다. 초판 수정의
  `approval_mode="never"`는 서버 설정 키가 아니라 조용히 무시됨을 실앱에서 재확인했다.
  전체 승인과 sandbox를 없애는 위험한 플래그는 쓰지 않으며, readonly에는 이 예외를 주지
  않는다.
...
### §5-4. 실측 — 멘션 턴을 TUI 로 돌리면 그 제약이 사라진다 (2026-09-08)

§5-3 의 판정("fd 0 의 정체 하나로 잰다")은 **여전히 옳다.** 바뀐 것은 그 fd 0 을 PTY 로
만들 수 있다는 사실이다: `-p` 를 버리고 멘션 턴도 TUI(`claude --session-id`/`-r`, 비 `-p`)로
띄우면 `stdinFile` 이 null 이 되고, `acceptsPtyInput` → `acceptsInput` → 서버 게이트가
**규칙을 하나도 안 바꾼 채** 입력을 연다. §5-3 마지막 문단이 예고한 "writer 를 여는 조건이
바뀔 뿐"의 세 번째 갈래다.

실물 CLI 로 직접 쟀다(macOS, claude 2.1.263, node-pty 로 러너와 같은 모양의 PTY):

| 확인한 것 | 결과 |
|---|---|
| bracketed paste(`ESC[200~ … ESC[201~`)로 여러 줄 프롬프트 주입 | **된다** — 3줄이 한 메시지로 들어간다 |
| `-p` 로 만든 세션을 TUI `-r <id>` 가 이어받기 | **된다** — 앞 턴의 사실이 유지되고 `No conversation found` 가 없다 |
| TUI 에서 `--permission-mode plan` | **받는다** |
...
적용되지 않는다 — TUI 는 stdin 이 tty 인 것이 정상이고, 이 저장소의 인터랙티브 턴
(`interactiveTurn.ts`, `stdinFile: null`)이 이미 그렇게 돌고 있다. `pty.ts::composeSpawn` 의
`#380` 절을 그 범위로 읽어야 한다.

**인터럽트에 새 표면이 필요 없다.** 데스크탑 xterm 이 보내는 `Ctrl+C` 가 릴레이를 타고 그대로
PTY 에 닿고, 그 뒤는 하네스가 한다. 즉 "진행 중인 턴을 사람이 멈춘다"를 위해 리액션 제스처·
서버 판정·릴레이 프레임·레지스트리의 kill 손잡이를 **하나도 만들지 않는다**.

**위험 — 자격증명 판정을 tail 에서 떼야 한다.** 주입한 프롬프트는 PTY 가 그대로 에코하므로
`runPtyTurn` 의 tail(2KB 링)에 본문이 섞인다. `policy.ts::isCredentialFailure` 는 그 tail 의
문자열만으로 판정하고, 참이면 `runnerExitPlan` 이 **러너를 78 로 물러나게 한다**. 그대로 두면
에이전트를 멘션할 수 있는 사람이 본문에 `authentication_error` 같은 문구를 넣는 것만으로 그
러너가 맡은 **모든 스레드**를 함께 죽일 수 있다. 이 저장소는 같은 오판 경로를 `#380` 2단계
실측에서 이미 관측했다(`test/policy.test.ts` "#380 1단계 실측").

TUI 는 턴마다 종료하지 않으므로 exit code 로 판정할 수도 없다 — **두 문제의 답이 하나다:
자격증명 실패 판정을 tail 문자열이 아닌 구조적 신호로 옮긴다.** 이것이 실행 모델 교체의
선행 조건이다.
