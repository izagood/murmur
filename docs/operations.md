...
**사이드카 배포로 이 함정의 표적이 바뀌었다.** 예전에는 못 찾는 것이 `pnpm` 이었고 러너가
아예 안 떴다. 지금 사이드카는 `PATH` 에서 찾는 대상이 아니라(앱이 그 경로를 직접 안다)
**러너는 뜬다** — 대신 러너가 턴마다 부르는 `claude`·`codex`·`avcs`·`git` 이 그 `PATH` 에서
발견돼야 한다. 하나라도 없으면 "러너는 살아 있는데 답을 못 하는" 조용한 실패가 된다(8-1 의
plist 함정과 같은 모양이다).

`#305` 가 만든 장치는 남아 있고 한 겹으로 줄었다:
...
2. **못 읽으면 `SYSTEM_PATH_FALLBACK`**(`/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`)으로
   물러난다. **여기서 기동을 막지 않는다** — 사이드카 자신은 이 값이 없어도 뜬다. 옛
   `pnpm path` 설정 칸이 있던 자리이고, `pnpm` 을 안 거치게 되면서 없앴다.
...
항목은 `sh` 에 `["-lc", "echo $PATH"]` **그 인자 배열 하나만** 허용한다 — 그리고 지금 그
항목이 스코프에 남은 `shell:allow-execute` 의 **전부**다(러너 spawn 이 빠져나갔다).
`args: true` 나 정규식 인자로 바꾸는 순간 웹뷰가 임의 명령을 실행할 수 있게 되고,
`test/runnerShellScope.test.ts` 가 그것을 막는다.
