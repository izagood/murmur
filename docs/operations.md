...
자식은 앱 프로세스의 환경을 물려받고 거기에 `MURMUR_PAT`·`MURMUR_URL` 만 더한다. macOS 에서
Finder/Dock 으로 띄운 앱의 `PATH` 는 로그인 셸의 것이 아니라 `/usr/bin:/bin:/usr/sbin:/sbin`
정도라 **`pnpm` 을 못 찾는 경우가 있다** — launchd 감독(8-1)이 겪는 그 함정과 같은 것이다.
그러면 상태가 '기동 실패' 로 남고 사유(spawn 오류)가 화면에 붙는다. 앱을 터미널에서
(`pnpm --filter @murmur/desktop tauri dev`, 또는 `open` 대신 셸에서 직접) 띄우면 로그인 셸의
`PATH` 를 물려받는다. `sh -lc` 로 감싸 로그인 셸을 통과시키는 길은 **쓰지 않는다** — 그것이
곧 shell 스코프를 와일드카드로 여는 것이다.
