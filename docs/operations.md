...
앱은 두 가지를 **갈라서** 말한다. 버전이 다른 것만으로 경고를 켜면 그 경고는 거의 항상
켜져 있고(릴리스는 하루에도 여러 번 돌고 서버는 손으로 재배포한다), 그러면 아무도 안 본다.

| 서버가 | 화면이 | 지금 할 일 |
|---|---|---|
| 앱과 같거나 앞선다 | 숫자만 | 없다 |
| 뒤처졌지만 **호환 하한은 넘는다** | 주의색 한 줄 — *"뒤처졌지만 요구 버전은 넘는다"* | 없다. 편할 때 재배포 |
| **호환 하한보다 낮다** | 화면 위쪽 **붉은 띠** + 줄이 고장색 — *"v{하한} 이상으로 재배포해라"* | **지금 재배포** |
| 버전을 안 싣는다 | 주의색 — *"버전을 싣기 전 판이다"* | 재배포하면 숫자가 보인다 |

**호환 하한**은 `packages/shared/src/compat.ts` 의 `MIN_SERVER_VERSION` 이다 — *"이 앱이
제대로 돌려면 서버가 최소 이 버전이어야 한다"*. 자동으로 오르지 않는다: 서버 표면에
기대는 것을 앱에 새로 넣은 사람이 그 PR 에서 함께 올리고, 무엇이 깨지는지를 그 파일의
표에 적는다(`CONTRIBUTING.md` §*When your change needs a newer server*). 하한 아래 서버는
**띠가 서므로 설정을 열지 않아도 보인다**(활성 커뮤니티 기준, 그 버전에 대해 닫을 수 있다).

지금 값과 근거는 그 파일 하나만 열면 된다. 서버에서 하한을 넘는지 눈으로 확인하려면:

```
curl -fsS http://localhost:3400/healthz | python3 -c 'import json,sys; print(json.load(sys.stdin)["version"])'
```
...
| 무엇 | 예전 | 지금 |
|---|---|---|
| 러너 실행 파일 | `pnpm --filter @harkroom/agent start`(소스) | **단일 번들**을 Tauri 사이드카(`externalBin`)로 앱과 함께 배포 |
