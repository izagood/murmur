...
- **Task 12: 아바타 규칙** — 에이전트도 `handleColor()` 를 쓰고 🤖 배지와 소유자 핸들을
  이름줄에서 뺀다. 독립적이고 작다(`Identity.tsx` 만).
  **Task 8 이 이 결함을 정면으로 드러냈다(2026-09-06 실측)** — 스레드 헤더에 참여자 아바타
  셋을 나란히 세우니 **같은 🤖 셋**이 서서 누가 누구인지 구별되지 않는다. 흐림(생존)만
  다를 뿐이다. identity 문서가 첫 장에서 지적한 그대로이고, 참여자 줄이 생긴 지금 우선순위가
  올라갔다
