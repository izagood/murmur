...
- ⚠️ 개발 머신 함정: **Claude Code 안에서 띄운 자식 claude 는 세션을 저장하지 않는다** —
  ...
  - **취소선 부분은 틀렸다(#374, 2026-09-04).** 근거 없이 적힌 판단이었고 코드가 그것을
    보장하지 않는다. `turn.ts::childEnv` 는 `process.env` 를 **통째로** 복사해 자식에게
    넘긴다(필터가 한 개도 없었다) — 즉 러너 env 에 무엇이 있든 그대로 하네스로 간다.
    그리고 러너가 어디서 뜨는지는 고정돼 있지 않다: 데스크탑 앱이 띄우고(#250
    `runnerLauncher.ts`), 그 앱이 Dock 에서 떴는지 터미널에서 떴는지에 따라 env 가
    다르다는 것은 #305 가 `PATH` 로 이미 실측했다. 도그푸딩 중에는 러너가 Claude Code
    세션 안에서 뜨고, **그 셸에 이 마커가 실제로 들어 있음을 확인했다**(`env | grep
    '^CLAUD'` → `CLAUDE_CODE_CHILD_SESSION` 포함 11개). 그래서 #374 가 `childEnv` 에
    `HARNESS_ENV_DENYLIST` 를 넣어 이 마커만 벗긴다.
  - 다만 **해악의 범위는 이 기록보다 좁다**(#374 에서 실측): 멘션 턴이 쓰는 `-p` 경로는
    마커가 있어도 세션 파일이 생겼다(`claude -p --session-id <uuid>` 를 마커가 살아 있는
    셸에서 실행 → `~/.claude/projects/<cwd>/<uuid>.jsonl` 62KB 생성 확인). 위 문장이
    말하는 "저장하지 않는다"는 **인터랙티브 TUI 경로**의 관찰이고, 그 경로는 #374 에서
    재현하지 못했다(`script`+파이프로는 TUI 가 배너까지 그리지 않았다) — 반증이 아니라
    **미확인**이다. 마커를 벗기는 판단은 "상속된다"는 확정 사실만으로 충분하고, 벗기는
    비용이 0 에 가깝기 때문이다.
...
## 실물 e2e 재현 절차 (#347, 2026-09-07 작성)

`#347` 이 남긴 실물 항목 다섯 개다. **"실물로 확인한다"만 적혀 있으면 다음 사람이 무엇을
어떻게 보는지 모른다** — 그래서 각각에 무엇을 준비하고, 무엇을 보고, 무엇이면 통과인지를
적는다. 각 항목 머리에 **왜 코드로는 못 재는지**를 함께 적었다(그것이 이 절차가 존재하는
이유이고, 다음 사람이 "테스트로 옮기면 되지 않나"를 다시 묻지 않게 한다).

### 공통 준비

- 러너가 붙은 에이전트 하나(claude 하네스). `entryPath` 관문을 통과한 workspace 가 있어야 한다.
- 데스크탑 앱에서 그 에이전트의 소유자로 로그인한다(attach 칩은 **소유자·admin 에게만** 보인다).
- 도그푸딩 스택(`:3400`)이 아닌 곳에서 한다.

### 1. §14 기준 3 — 진행 중 턴의 실제 화면

**왜 코드로 못 재는가:** 바이트가 `TerminalPanel` 까지 닿는 것은 단위·계약 테스트가 이미
덮는다. 못 재는 것은 **그 바이트가 화면에 무엇으로 그려지는가**다 — 셀 크기·폰트 메트릭·
ANSI 색은 xterm 의 `Terminal#open` 이 캔버스와 `getComputedStyle` 을 만져야 정해지는데
jsdom 에는 레이아웃 엔진이 없다(`terminalSink.ts` 파일 머리가 이음새를 둔 이유가 그것이다).
실제로 `new Terminal` 은 **어느 테스트에서도 인스턴스화되지 않는다**.

1. 스레드에서 에이전트를 멘션해 턴을 띄운다.
2. 턴이 도는 동안 [▶ 터미널] 칩을 눌러 attach 한다.
3. **본다:** 하네스 TUI 가 터미널에서 직접 볼 때와 같은 모양인가 —
   - 박스 그리기 문자가 깨지지 않고 이어지는가(폰트가 등폭인가)
   - 색이 나오는가(ANSI). 회색 한 겹으로 뭉개지지 않는가
   - **줄이 접히거나 잘리지 않는가** — `#335` 가 지목한 자리다. 창을 좌우로 끌어
     크기를 바꾸고, TUI 가 그 크기로 다시 그리는지 본다(SIGWINCH 가 닿는지)
4. **통과 조건:** 같은 하네스를 터미널에서 직접 띄운 화면과 나란히 놓고 **같아 보인다**.

### 2. §14 기준 4 — attach 로 고친 것을 다음 멘션 턴이 안다

**왜 코드로 못 재는가:** 왕복의 각 조각(인터랙티브 기동·`turnsRun` 판정·세션 파일 존재
검사)은 단위로 덮인다. 못 재는 것은 **실제 하네스가 그 세션을 정말 이어받는가**다 —
스파이크 §2 가 실측한 `Session ID is already in use` / `No conversation found` 는 CLI 가
내는 것이고, 그 판정이 한 칸만 어긋나도 다음 턴이 죽는다. 스펙 §10 이 기록한
"단위 21건 초록인데 CLI 가 죽은" 것과 같은 층이다.

1. 스레드에서 [터미널 열기] 를 눌러 인터랙티브 세션을 연다.
2. 그 안에서 하네스에게 **실제로 한 턴을 시킨다** — 파일 하나를 고치게 한다
   (예: workspace 에 `MARK.txt` 를 만들고 "딸기 파인애플" 을 쓰게 한다).
   **대화를 실제로 해야 한다** — 열었다 그냥 닫으면 세션 파일이 안 생기고(스파이크 §2),
   그때는 `turnsRun` 이 올라가지 않는 것이 **정상**이다.
3. 터미널을 닫는다.
4. **같은 스레드에** 멘션한다: "방금 만든 파일에 뭐라고 썼지?"
5. **통과 조건:** 답이 `딸기 파인애플` 을 정확히 말한다. 즉 멘션 턴이 `-r` 로 그 대화를
   이어받았다. 실패 모양 둘을 구별하라 — "모른다"고 답하면 세션이 안 이어진 것이고,
   턴이 **죽으면** `turnsRun` 판정이 틀린 것이다(위 두 에러 메시지를 러너 로그에서 본다).

### 3. §14 기준 10 — 둘이 **동시**·한 스레드

**왜 단위 테스트가 못 잡는가 — 여기가 스펙 §10 의 함정 자리다.** 러너의 큐잉·클램프·
프롬프트 조립은 전부 초록이고, 그것들은 `seq` 를 **주어진 값**으로 받는다. 깨지는 곳은
그 위층이다: 에이전트마다 **러너 프로세스가 따로**이고(daemon 이 에이전트당 하나를 띄운다)
**둘 사이를 직렬화하는 잠금이 서버에도 러너에도 없다**. 각자 자기 `lastFedSeq` 커서를
자기 파일에 들고 독립적으로 폴한다.

그 위에 `message.seq` 는 채널별이 아니라 전역 `generated always as identity` 이고,
identity 는 트랜잭션 밖에서 값을 뽑는다 — **seq 를 먼저 받은 트랜잭션이 나중에 커밋할 수
있다.** 이 조합의 서버 쪽 절반은 이제 잰다:
[`packages/server/test/concurrentThreadCursor.test.ts`](../../packages/server/test/concurrentThreadCursor.test.ts)
가 트랜잭션 둘을 실제로 겹쳐 **낮은 seq 가 늦게 보이는 창**과 **그 창에서 전진한 커서가
동료의 발화를 영원히 건너뛰는 것**을 재현한다(순차 대조군 포함).

**실물로 남은 것은 그 창이 실제 운영에서 열리는 빈도와 결과다** — 위 테스트는 창을 손으로
열어 보인 것이고, 러너 둘이 자연히 그 창에 빠지는지는 실물로만 안다.

1. 러너 **둘**을 띄운다 — 각자 다른 PAT·`AGENT_STATE_DIR`·**다른 workspace**.
   (같은 workspace 면 기준 10 의 "각자 workspace" 가 성립하지 않는다.)
2. **한 메시지에 둘을 함께 멘션한다.** 순차로 부르면 안 된다 — 로드맵 §5 가 구분하듯
   순차는 2026-09-02 에 이미 닫혔고, 이 기준이 요구하는 것은 **동시**다.
3. **본다:** 둘이 같이 답하는가. 그리고 답이 다 올라온 뒤 **다시 한 번 둘을 멘션해**
   각자에게 "상대가 방금 뭐라고 했지?" 를 묻는다.
4. **통과 조건:** 각자가 **상대의 발화를 인용**한다. 한쪽이 상대의 답을 모른다고 하면
   위 커서 건너뜀이 실물에서 일어난 것이다 — 그때 러너 상태의 `lastFedSeq` 와 그 스레드의
   `seq` 를 비교해 **건너뛴 seq 가 있는지** 확인하라(있으면 소실이 아니라 커서 문제다).

### 4. 멘션 턴 attach 중 타이핑 왕복

**왜 코드로 못 재는가:** writer 규칙은 서버가 정하고 서버 단위 테스트가 덮는다. 못 재는
것은 **멘션 턴의 fd 0 이 PTY 가 아니라 파일**이라는 사실의 실물 귀결이다 — 그래서 이 턴은
`acceptsInput=false` 이고, 화면은 "여기는 못 친다"를 스스로 말해야 한다.

1. 멘션으로 턴을 띄우고, 도는 동안 attach 한다.
2. **본다:** 배지가 **읽기 전용**인가. 커서가 깜빡이지 않는가.
3. 키를 쳐 본다.
4. **통과 조건:** 아무것도 안 찍힌다. **찍히는데 러너에 안 닿는 것이 최악이다** —
   사람은 쳤다고 믿는데 아무 일도 일어나지 않는다.
5. 창을 **둘** 열어 승계도 같이 본다: 두 번째가 첫 번째를 강등시키고, 첫 번째를 닫으면
   남은 창이 승계하는가(인터랙티브 세션에서 봐야 배지가 "입력 가능" 으로 뜬다).

### 5. 조종 중 멘션 큐잉 — **클램프**

**왜 단위 테스트가 못 잡는가:** 클램프 자체(`Math.min(maxSeq, minPending - 1)`)는 단위 3건이
경계까지 덮는다(전진·클램프·되돌리지 않음). 못 재는 것은 **그 뒤**다 — 유예된 멘션이
inbox 의 at-least-once 로 **정말 다시 나오는가**, 그리고 풀려난 그 턴의 델타 프롬프트가
**비어 있지 않은가**. 큐가 서버 inbox 이고 클램프가 러너 메모리라 그 이음새는 두 프로세스에
걸쳐 있다.

1. [터미널 열기] 로 인터랙티브를 연다(닫지 않는다).
2. 그 스레드에 **멘션을 두 번** 보낸다.
3. **본다:** 스레드에 `(지금 …이 직접 조종 중입니다 — 이 멘션은 대기 N건째로 …)` 통지가
   **멘션당 한 번씩만** 뜨는가. 폴링마다 도배되면 안 된다. N 이 1, 2 로 는다.
4. 터미널을 닫는다.
5. **통과 조건:** 밀린 멘션 **둘 다** 처리되고, **각각의 답이 그 멘션의 내용에 맞다.**
   여기가 클램프의 자리다 — 클램프가 안 먹으면 델타가 비어 그 부름이 **조용히 소실**되거나
   엉뚱한 것에 답한다. "답이 왔다"가 아니라 **"그 멘션에 답했다"** 를 확인하라.
