...
- 아직 없는 것: **핸드셰이크 Origin 검증**과 **소켓 수명 ↔ 토큰 수명 정합**. WebSocket 핸드셰이크는
  CORS 의 보호를 받지 않으므로, 쿠키 인증으로 옮긴다면 Origin 허용 목록이 필수다(현재는 티켓
  방식이라 교차 출처 페이지가 티켓을 얻을 수 없다). 또한 토큰은 연결 시점에만 검증되므로,
  세션 만료나 PAT 폐기가 **이미 열린 소켓을 끊지 못한다**.
- 에이전트: admin이 발급하는 Bearer PAT(account 귀속). `DELETE /accounts/:id/pats/:label`로
  폐기(라벨 단위 — `pat.label`에 유일성이 없어 같은 라벨이 여러 개면 전부 폐기된다).
- **폐기가 실제 폐기가 된다.** `POST /auth/logout`은 그 세션 행을 지우고(이 토큰만 — 다른 기기는
  유지), 열린 WS 소켓은 15초 주기 sweep으로 자기 자격증명을 재검증해 무효면 `4401`로 끊긴다.
  토큰을 핸드셰이크 때만 검증했기 때문에, 이전에는 세션 만료·PAT 폐기가 **이미 열린 소켓을
  끊지 못했다**. 티켓이 발급 토큰의 해시를 들고 있어 소켓은 자기 자격증명을 정확히 안다
  (계정 단위가 아니다 — 다른 세션이 살아 있어도 폐기된 그 소켓만 죽는다).
  sweep은 DB 왕복 실패 시 **fail-open**이다: 일시적 장애가 전원 강제 로그아웃이 되면 안 된다.
