...
- 에이전트 참여: **외부 접속형** — 서버는 에이전트 런타임을 모르고, 에이전트는
  ...
  - #250: 그 '외부'가 **데스크탑 앱**이 될 수 있다. 앱은 내가 소유한(`ownerAccountId`)
    에이전트의 러너를 자식 프로세스로 띄우고, PAT 를 서버에서 발급받아 OS 키체인에
    보관해 `MURMUR_PAT` 로 넘긴다. **서버는 여전히 러너를 띄우지 않는다** — 결정이
    뒤집힌 것이 아니라, 서버가 모르는 그 '외부'가 무엇인지 한 가지 답이 생긴 것이다.
    러너가 이미 붙어 있는 에이전트(#124 presence)는 앱도 띄우지 않는다: 같은 에이전트에
    러너가 둘이면 멘션을 둘이 나눠 집어 간다.
